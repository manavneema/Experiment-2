...
def apply_human_priors(skeleton_edges, blacklist, whitelist):
    """Apply human priors to skeleton edges."""
    kept = []
    removed = []
    ...
    for a, b in whitelist:
        if (a, b) not in edge_pairs and (b, a) not in edge_pairs:
            new_edge = {
                "from": a,
                "to": b,
                "partial_corr": 0.5,  # Default strength
                "abs_partial_corr": 0.5,
                "type": "whitelisted",
                "source": "human_prior"
            }
...
if graphical_lasso_result['method'] == 'graphical-lasso-error':
    ...
    failure_artifacts = {
        ...
        "status": "FAILED",
        "failure_reason": graphical_lasso_result['error'],
...
print(f"\n[Step 9] Visualizing causal skeleton...")
G = visualize_skeleton(filtered_edges, title="Pipeline B: Graphical Lasso-Based Causal Graph")
...
print("\n[Step 12] GraphicalLasso edges (no Granger analysis needed)...")
...
artifacts = {
    ...
    "graphical_lasso_result": {
        ...
        "edges_found": len(graphical_lasso_result.get("edges", [])),
        "edge_weights_stats": graphical_lasso_result.get("edge_weights_stats", {})
    },
    "edge_stats": {
        "undirected_edges": len(filtered_edges),
        "edge_type": "partial_correlation",
        "orientation": "none_applied"
    },
...
cand_df = pd.DataFrame(rows).sort_values("abs_partial_corr", ascending=False)
cand_df.to_csv(f"{pipeline_path}/graphical_lasso_edges.csv", index=False)
...
print(f"  - graphical_lasso_edges.csv")
print(f"  - baseline_stats.json")
print(f"  - upstream_map.json")
print(f"  - downstream_map.json")
...
print(f"  - Partial correlation edges: {len(undirected_edges)}")
print(f"  - Final graph edges: {len(filtered_edges)}")
print(f"  - Connected nodes: {len(set([e['from'] for e in filtered_edges] + [e['to'] for e in filtered_edges]))}")
