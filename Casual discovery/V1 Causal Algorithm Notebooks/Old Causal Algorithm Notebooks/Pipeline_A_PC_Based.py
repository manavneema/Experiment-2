...
def apply_human_priors(skeleton_edges, blacklist, whitelist):
    """Apply human priors to skeleton edges strictly."""
    kept = []
    removed = []
    ...
    for a, b, edge_type in skeleton_edges:
        if (a, b) in blacklist:
            removed.append((a, b, edge_type, 'blacklisted'))
            print(f"  Removed blacklisted edge: {a} -> {b}")
        else:
            kept.append((a, b, edge_type))
    
    # Add whitelisted edges if missing
    added = []
    for a, b in whitelist:
        if (a, b) not in present:
            added.append((a, b, 'whitelisted', 'whitelisted'))
            kept.append((a, b, 'whitelisted'))
...
if pc_result['method'] == 'pc-error':
    ...
    failure_artifacts = {
        ...
        "status": "FAILED",
        "failure_reason": pc_result['error'],
...
# Step 6: Extract edges from PC result
print(f"\n[Step 6] Extracting edges from PC result...")
pc_edges = pc_result.get('edges', [])
print(f"PC discovered {len(pc_edges)} edges")

# Step 7: Apply human priors
print(f"\n[Step 7] Applying human priors...")
filtered_edges, review = apply_human_priors(
    pc_edges,
    blacklist=HUMAN_PRIOR_BLACKLIST,
...
print(f"\n[Step 8] Visualizing causal skeleton...")
G = visualize_skeleton(pc_edges, title="Pipeline A: PC-Based Causal Graph")
...
print("\n[Step 10] Exporting all artifacts...")
...
artifacts = {
    ...
    "pc_result": {
        ...
        "edges_found": len(pc_result.get("edges", []))
    },
    "filtered_edges": filtered_edges,
    "human_priors_review": review,
    "final_graph_stats": {
...
dbutils.fs.put(f"{pipeline_path}/feature_selection_log.json", 
               json.dumps(feature_selection_log, indent=2), 
               overwrite=True)

# Create simple candidate list from PC edges for compatibility
rows = []
...
cand_df.to_csv(f"{pipeline_path}/causal_candidates_pc.csv", index=False)

print("="*80)
print("✓ PIPELINE A COMPLETE — All artifacts saved")
print("="*80)
print(f"\nSaved artifacts to: {pipeline_path}")
print(f"  - causal_artifacts.json")
print(f"  - causal_metrics_matrix.csv")
print(f"  - causal_candidates_pc.csv")
print(f"  - feature_selection_log.json")
print(f"  - baseline_stats.json")

print(f"\nFinal Results:")
print(f"  - Training period: {MAX_RUNS_TO_PIVOT} days")
print(f"  - Initial features: {feature_selection_log['initial_features']}")
print(f"  - Final features: {feature_selection_log['final_features']}")
print(f"  - Feature reduction: {feature_selection_log['reduction_ratio']:.1%}")
print(f"  - Sample-to-feature ratio: {final_features.shape[0]/final_features.shape[1]:.2f}")
print(f"  - PC edges discovered: {len(pc_edges)}")
print(f"  - Final graph edges: {len(filtered_edges)}")